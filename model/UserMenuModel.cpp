...
UserMenuModel::UserMenuModel(const bool) {
...
auto UserMenuModel::comparePasswords(const int param, const Password &pwd1, const Password &pwd2) -> int {
    const auto by_name = [](auto &inner_pwd1, auto &inner_pwd2) {
        return inner_pwd1.getName() < inner_pwd2.getName();
    };
    const auto by_password = [](auto &inner_pwd1, auto &inner_pwd2) {
        return inner_pwd1.getPassword() < inner_pwd2.getPassword();
    };
    const auto by_category = [](auto &inner_pwd1, auto &inner_pwd2) {
        return inner_pwd1.getCategory() < inner_pwd2.getCategory();
    };
    const auto by_website = [](auto &inner_pwd1, auto &inner_pwd2) {
        return inner_pwd1.getWebsite() < inner_pwd2.getWebsite();
    };
    const auto by_login = [](auto &inner_pwd1, auto &inner_pwd2) {
        return inner_pwd1.getLogin() < inner_pwd2.getLogin();
    };

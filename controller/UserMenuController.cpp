...
#include "../model/Password/Generator.h"
...
auto UserMenuController::userMenu() -> void {
    while (true) {
        ...
        const auto option = getSingleOption(10);
...
auto UserMenuController::editPassword() -> void {
    ...
    passwordTable();
    const auto index = getSingleOption(static_cast<int>(userMenuModel.getPasswords().size())) - 1;
    user::displayEditPasswordAttribute();

    switch (getSingleOption(5)) {
        case 1:
...
auto UserMenuController::deletePassword() -> void {
    ...
    passwordTable();
    auto indexes = getMultiOption(static_cast<int>(userMenuModel.getPasswords().size()));
...
auto UserMenuController::addCategory() -> void {
    ...
    if (const auto category_name = getStringInput(); userMenuModel.isNameInCategories(category_name)) {
...
auto UserMenuController::deleteCategory() -> void {
    ...
    const auto index = getSingleOption(static_cast<int>(userMenuModel.getCategories().size())) - 1;
    user::displayDeleteList();
...
auto UserMenuController::viewTimestamp() const -> void {
    user::displayViewTimestamp(userMenuModel.getTimestamp());
}

auto UserMenuController::saveAndClose() const -> void {
...
auto UserMenuController::passwordTable(const std::string &query) const -> void {
    auto password_table = std::vector<std::vector<std::string> >();
...
auto UserMenuController::passwordCategory() const -> std::string {
...
auto UserMenuController::checkCategoriessEmpty() const -> bool {
